...
def get_posts_from_current_page(driver, cache):
    ...
    for post_element in posts_elements:
        user_name_element = post_element.find_element(By.XPATH, ".//a[contains(@href, 'mode=viewprofile')]")
        user_name = user_name_element.text
        user_id = parse_user_id_from_profile_link(user_name_element.get_attribute('href'))
        ...
        html = content_element.get_attribute('innerHTML')  # or .text, depending on how you want to handle the content
        ...
        post = ThreadPost(author=author, text=None, content_html=html, attachbox_html=attachbox_html, timestamp=timestamp)
        posts.append(post)
...
def log_forum_thread(thread_url, driver, cache):
  ...
  page_urls = [thread_url] + [link.get_attribute('href') for link in pagination_links if link.get_attribute('href') not in thread_url]

  # Remove duplicates and already visited page
  page_urls = list(dict.fromkeys(page_urls))  # Remove duplicates
  page_urls.remove(thread_url)  # Remove the first page URL as it's already processed
...
def test_step_1_log_forum_thread(driver: WebDriver):
  ...
  ft = log_forum_thread(
    "https://forum.image-line.com/viewtopic.php?t=310058",
    driver,
    cache)
...
def test_step_1_snippet_acquisition(driver: WebDriver):
  ...
  ft = log_forum_thread(
    'https://forum.image-line.com/viewtopic.php?t=317766',
    driver,
    cache)
...
def step_1_log_forum_threads(driver: WebDriver):
  ...
  for page in PianorollPageIterator(page_limit=4):
    ...
    thread_links = [x.get_attribute("href") for x in driver.find_elements(By.CLASS_NAME, "topictitle")]
