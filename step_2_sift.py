  ...
  for item in input_lists:
    ...
    if link_match:
      ...
      if label_match:
        ...
        versions.append(VersionProcessed(label=label,
                                         link=full_link,
...
def step_2_sift():
  ...
  for k in cache.iterkeys():
    ...
    if any([re.search(p, ft.posts[0].author.user_name) for p in ignore_authors]):
      continue
    ...
    elements_to_remove = soup.find_all(attrs={"style": lambda x: x and 'width: 100%; height: 100%; background-image: url("data:image/png;base64' in x})
