...
def construct_header(data):
  ...
  result = subprocess.run(['gomplate', '-f', header_template_path, '-c', f'.={tmpjson}'], capture_output=True, text=True)
...
def prompt_all(header_spec_input, **kwargs):
  ...
  for field in fields(header_spec_input):
    if field.metadata.get("prompt", False):  # Check if this field is flagged for prompting
...
def step_5_header_tag():
  ...
  for k in cache.iterkeys():
    ...
    for v in ftp.versions:
      ...
      labels = [(pick_version_label(f, v, d=' / '), f) for f in list_files_recursive(vpath)]
      ...
      for normalized_label, file_path in labels:
        ...
        prompt_all(
          header_spec_input=header_spec_input,
          ftp=ftp,
